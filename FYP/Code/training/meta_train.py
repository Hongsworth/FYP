...
from nets.attention_dnn import attention_dnn
from utils.funcs import to_complex

def multi_task_learning(args, net, h_list_meta, writer_meta_training, Noise, noise_var):
    ...
    for epochs in range(args.num_epochs_meta_train):
        ...
        for ind_meta_dev in range(args.tasks_per_metaupdate):
            ...
            if args.if_attention:
                iter_in_sampled_device, first_loss_curr, second_loss_curr = attention(args, iter_in_sampled_device,
                                                                                 net, current_channel, Noise, noise_var)
            else:
...
def attention(args, iter_in_sampled_device, net, current_channel, Noise, noise_var):
    net.zero_grad()
    para_list_from_net = list(map(lambda p: p[0], zip(net.parameters())))
    net_meta_intermediate_attention = attention_dnn(if_relu = args.if_relu)
    m, label = message_gen(args.bit_num, args.mb_size_meta_test)
    m = m.type(torch.FloatTensor).to(args.device)
    label = label.type(torch.LongTensor).to(args.device)
    out = net_meta_intermediate_attention(m, para_list_from_net, args.if_bias, current_channel, args.device, Noise, args.if_RTN, args.if_true_snr, noise_var)
    loss = torch.nn.functional.cross_entropy(out, label)
    first_loss_curr = float(loss)
    grad = torch.autograd.grad(loss, para_list_from_net, create_graph=False)
    second_loss_curr = first_loss_curr
