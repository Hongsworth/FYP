...
from nets.att_autoencoder import att_dnn
...
import datetime
import os
from utils.funcs import to_complex
...
def parse_args():
    ...
    parser.add_argument('--if_joint_training', dest='if_joint_training', action='store_true', default=True) # else: meta-learning for multi-task learning
    # if attention module is used
    parser.add_argument('--if_attention', dest='if_attention', action='store_true', default=True)
    ...
    parser.add_argument('--if_adam_after_sgd', dest='if_adam_after_sgd', action='store_true',
                        default=False)
    parser.add_argument('--if_64bit', dest='if_64bit', action='store_true', default = False)

    parser.add_argument('--if_channel_estimation', dest='if_channel_estimation', action='store_true', default=False)

    parser.add_argument('--if_true_snr', dest='if_true_snr', action='store_true', default=True)
...
if __name__ == '__main__':
    ...
    if args.if_joint_training:
        ...
        pilot = torch.tensor([-0.5,0.5,0.5,-0.5,0.5,-0.5,0.5,-0.5,-0.5,0.5,0.5,-0.5,0.5,0.5,0.5,0.5,-0.5,0.5,0.5,-0.5,0.5,-0.5,0.5,-0.5,0.5,0.5,-0.5,-0.5,0.5,0.5,-0.5,0.5], dtype=torch.float)
    pilot = pilot.view(1, -1)
    pilot_complex = to_complex(pilot)

    actual_channel_num = args.channel_num * 2
    if args.if_attention:
        net = att_dnn(M=pow(2, args.bit_num), num_neurons_encoder=args.num_neurons_encoder, n=actual_channel_num, n_inv_filter = args.tap_num,
    ...
    if args.if_attention:
        net_for_testtraining = att_dnn(M=pow(2, args.bit_num), num_neurons_encoder=args.num_neurons_encoder, n=actual_channel_num, n_inv_filter = args.tap_num,
                num_neurons_decoder=args.num_neurons_decoder, if_bias=args.if_bias, if_relu=args.if_relu, if_RTN=args.if_RTN)
    else:
    ...
    if args.path_for_meta_trained_net is None:
        ...
        multi_task_learning(args, net, h_list_meta, writer_meta_training, Noise, noise_var)
    ...
    for adapt_steps in test_adapt_range:
        ...
        for h in h_list_test:
            ...
            test_training(args, h, net_for_testtraining, Noise, PATH_before_adapt, PATH_after_adapt, adapt_steps, noise_var)
            # test
            ind_snr = 0
            for test_snr in test_Eb_over_N_range:
                block_error_rate_per_snr_per_channel = test_per_channel_per_snr(args, h, net_for_testtraining, test_snr, actual_channel_num, PATH_after_adapt, if_val, noise_var)
